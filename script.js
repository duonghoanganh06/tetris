document.addEventListener("DOMContentLoaded", () => {
  ...
  function freeze() {
    if (
      current.some((index) =>
        squares[currentPositon + index + width].classList.contains("taken")
      )
    ) {
      current.forEach((index) =>
        squares[currentPositon + index].classList.add("taken")
      );
  ...
  function moveLeft() {
    ...
    const isAtLeftEdge = current.some(
      (index) => (currentPositon + index) % width === 0
    );
    ...
    if (
      current.some((index) =>
        squares[currentPositon + index].classList.contains("taken")
      )
    ) {
      currentPositon += 1;
    }
  ...
  function moveRight() {
    ...
    const isAtRightEdge = current.some(
      (index) => (currentPositon + index) % width === width - 1
    );
